...
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_Disable, ENABLE);	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7;	// SPI_SCK, SPI_MOSI
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
...
* Attention		 : None
*******************************************************************************/
//u8 _spi_read_write(uint8_t data)
//{
//	u8 Data = 0;
...
//	SPI1_Configuration();
	...
	if(retry == 0xFFF)
	{
		...
		USART_SendString(USART1, "Reset card into IDLE state failed!\r\n");
#endif
		return 1;
	}
	...
	if(r1 == 0x05)
	{
	  ...
	  for(retry=0; retry<0xFFF; retry++)
	  {
		 ...
		 if(r1 != 0x01)
		 {
			...
			USART_SendString(USART1, "Send CMD55 should return 0x01, response=0x%02x\r\n"); 
#endif
			return r1;
		 }
	  ...
	  if(retry == 0xFFF)
	  {
		 ...
		 if(retry == 0xFFF)
		 {
			...
			//printf("Send CMD1 should return 0x00, response=0x%02x\r\n", r1);
			 USART_SendString(USART1, "1Send CMD55 should return 0x01, response=0x%02x\r\n"); 
#endif
			return 2;
		 }	
